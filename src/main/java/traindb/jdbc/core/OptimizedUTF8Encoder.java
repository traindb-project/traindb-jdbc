...
abstract class OptimizedUTF8Encoder extends Encoding {
   ...
   *
   * @param data              The {@code byte[]} to decode.
   * @param offset            The starting index in <i>data</i>.
   * @param length            The number of bytes in <i>data</i> to decode.
   * @param codePointConsumer The consumer of all decoded code points.
   * @throws IOException If data is not valid utf-8 content.
   */
  static String decodeToChars(byte[] data, int offset, int length, char[] chars, int out)
      throws IOException {
    int in = offset;
      ...
      while (in < end) {
        ...
        if (ch < 0x80) {
          ...
          throw new IOException(
              MessageFormat.format("Illegal UTF-8 sequence: initial byte is {0}: {1}", "10xxxxxx",
                  ch));
        } else if (ch < 0xe0) {
          // 110xxxxx 10xxxxxx
          ch = ((ch & 0x1f) << 6);
          ...
          throw new IOException(
              MessageFormat.format("Illegal UTF-8 sequence: initial byte is {0}: {1}", "11111xxx",
                  ch));
        }

        if (ch > MAX_CODE_POINT) {
          throw new IOException(
              MessageFormat.format("Illegal UTF-8 sequence: final value is out of range: {0}", ch));
        }
        ...
        if (ch > 0xffff) {
          ...
          throw new IOException(
              MessageFormat.format("Illegal UTF-8 sequence: final value is a surrogate value: {0}",
                  ch));
        } else {
          // Normal case.
          chars[out++] = (char) ch;
    ...
    if ((ch & 0xc0) != 0x80) {
      throw new IOException(
          MessageFormat.format(
              "Illegal UTF-8 sequence: byte {0} of {1} byte sequence is not 10xxxxxx: {2}", pos,
              len, ch));
    }
  }
  ...
  private static void checkMinimal(int ch, int minValue) throws IOException {
    ...
    throw new IOException(MessageFormat.format(
        "Illegal UTF-8 sequence: {0} bytes used to encode a {1} byte value: {2}", actualLen,
        expectedLen, ch));
  }

  /**
   * Returns a {@code char[]} to use for decoding. Will use member variable if
   * <i>size</i> is small enough. This method must be called, and returned
   * {@code char[]} only used, from {@code synchronized} block.
   *
   * @param size The needed size of returned {@code char[]}.
   * @return A {@code char[]} at least as long as <i>length</i>.
   */
  char[] getCharArray(int size) {
    ...
    // only if size is below the threshold do we want to keep new char[] for future
    // reuse
  ...
  /**
   * Decodes binary content to {@code String} by first converting to
   * {@code char[]}.
   */
