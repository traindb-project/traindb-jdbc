...
/**
 * A faster version of BufferedInputStream. Does no synchronisation and allows
 * direct access to the used byte[] buffer.
 *
...
public class VisibleBufferedInputStream extends InputStream {
	...
	/**
	 * If a direct read to byte array is called that would require a smaller read
	 * from the wrapped stream that MINIMUM_READ then first fill the buffer and
	 * serve the bytes from there. Larger reads are directly done to the provided
	 * byte array.
	 */
	...
	/**
	 * Reads byte from the buffer without any checks. This method never reads from
	 * the underlaying stream. Before calling this method the {@link #ensureBytes}
	 * method must have been called.
	 *
	 * @return The next byte from the buffer.
	 * @throws ArrayIndexOutOfBoundsException If ensureBytes was not called to make
	 *                                        sure the buffer contains the byte.
	 */
	public byte readRaw() {
		return buffer[index++];
	}
	...
	/**
	 * Ensures that the buffer contains at least n bytes. This method invalidates
	 * the buffer and index fields.
	 *
	 ...
	 * @return true if required bytes are available and false if EOF or the
	 *         parameter block was false and socket timeout occurred.
	 * @throws IOException If reading of the wrapped stream failed.
	 */
	...
	/**
	 * Moves bytes from the buffer to the beginning of the destination buffer. Also
	 * sets the index and endIndex variables.
	 *
	...
	/**
	 * Returns direct handle to the used buffer. Use the {@link #ensureBytes} to
	 * prefill required bytes the buffer and {@link #getIndex} to fetch the current
