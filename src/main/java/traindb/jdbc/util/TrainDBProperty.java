...
public enum TrainDBProperty {
   ...
   */
  TRAINDB_DBNAME("TrainDBNAME", null,
   ...
   */
  TRAINDB_HOST("TrainDBHOST", null,
      "Hostname of the TrainDB server (may be specified directly in the JDBC URL)", false),

  /**
   * Port of the PostgreSQL server (may be specified directly in the JDBC URL)
   */
  TRAINDB_PORT("TrainDBPORT", null,
      "Port of the PostgreSQL server (may be specified directly in the JDBC URL)"),
  ...
  public int getInt(Properties properties) throws TrainDBJdbcException {
    ...
    try {
      ...
      throw new TrainDBJdbcException(
          MessageFormat.format("{0} parameter value must be an integer but was: {1}",
