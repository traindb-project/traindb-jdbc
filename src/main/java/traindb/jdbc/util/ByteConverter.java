...
public class ByteConverter {
   ...
   *
   * @param bytes    array of bytes to be decoded from binary numeric
   *                 representation.
   * @param pos      index of the start position of the bytes array for number
   * @param numBytes number of bytes to use, length is already encoded in the
   *                 binary format but this is used for double checking
  ...
  public static Number numeric(byte[] bytes, int pos, int numBytes) {
    ...
    // An integer should be built from the len number of 2 byte shorts, treating
    // each
    // as 4 digits.
    // The weight, if > 0, indicates how many of those 4 digit chunks should be to
    // the
    // "left" of the decimal. If the weight is 0, then all 4 digit chunks start
    // immediately
    // to the "right" of the decimal. If the weight is < 0, the absolute distance
    // from 0
    // indicates 4 leading "0" digits to the immediate "right" of the decimal, prior
    // to the
    // digits from "len".
    // A weight which is positive, can be a number larger than what len defines.
    // This means
    // there are trailing 0s after the "len" integer and before the decimal.
    // The scale indicates how many significant digits there are to the right of the
    // decimal.
    // A value of 0 indicates a whole number (integer).
    // The combination of weight, len, and scale can result in either trimming
    // digits provided
    // by len (only to the right of the decimal) or adding significant 0 values to
    // the right
    ...
    if (scale == 0) {
      ...
      // the binary representation, where weight and len can infer trailing 0s, can
      // result in a negative scale
      // to produce a consistent BigDecimal, we return the equivalent object with
      // scale set to 0
      return bigDecScale == 0 ? new BigDecimal(unscaledBI) :
   ...
   * {@link org.postgresql.core.Oid#NUMERIC}.
   *
  ...
  public static long int8(byte[] bytes, int idx) {
    return ((long) (bytes[idx + 0] & 255) << 56) + ((long) (bytes[idx + 1] & 255) << 48)
        + ((long) (bytes[idx + 2] & 255) << 40) + ((long) (bytes[idx + 3] & 255) << 32)
        + ((long) (bytes[idx + 4] & 255) << 24) + ((long) (bytes[idx + 5] & 255) << 16)
        + ((long) (bytes[idx + 6] & 255) << 8) + (bytes[idx + 7] & 255);
  }
  ...
  public static int int4(byte[] bytes, int idx) {
    return ((bytes[idx] & 255) << 24) + ((bytes[idx + 1] & 255) << 16) +
        ((bytes[idx + 2] & 255) << 8)
        + ((bytes[idx + 3] & 255));
  }
